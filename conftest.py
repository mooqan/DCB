...
def pytest_configure(config):
    if not hasattr(config, "slaveinput"):
        ...
        ensure_dir("results")
        ensure_dir(os.path.join("allureResult", current_day))
        result_dir = os.path.join(os.path.dirname(__file__), "allureResult", current_day)
...
def device_logger(request):
    ...
    return DeviceLogger(logcat_dir)
